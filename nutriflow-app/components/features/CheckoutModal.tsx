...
import { X, ShieldCheck, Crown, Sparkles, CheckCircle2, Lock, Loader2, Receipt, Download, ArrowRight } from 'lucide-react';
              ...
              <div className="flex-1 p-10">
                ...
                <div className="mt-10 pt-10 border-t border-slate-100 dark:border-slate-800 text-center">
                   <div className="flex items-center justify-center gap-3 opacity-40">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest">{tr('checkout_encrypted')}</span>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-10 md:p-14 text-center">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 p-8 mb-10 text-left relative overflow-hidden group">
                ...
                <div className="grid grid-cols-2 gap-y-6 relative z-10">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{tr('checkout_receipt_id')}</p>
                    <p className="text-sm font-black text-slate-900 dark:text-slate-100">{receiptData.id}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{tr('checkout_date')}</p>
                    <p className="text-sm font-black text-slate-900 dark:text-slate-100">{receiptData.date}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{tr('checkout_selected_plan')}</p>
                    <p className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase">{planName}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{tr('checkout_amount_paid')}</p>
                ...
                <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{tr('checkout_verified_payment')}</span>
                  </div>
                  <Button variant="ghost" size="sm" className="gap-2 text-[10px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400">
                    <Download className="h-3 w-3" /> {tr('checkout_save_pdf')}
                  </Button>
                </div>
              </div>
              ...
              >
                {tr('checkout_go_dashboard')} <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </div>
          )}
        </motion.div>
