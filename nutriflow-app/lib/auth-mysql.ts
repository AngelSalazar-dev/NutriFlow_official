...
export async function getSessionCookie(): Promise<string | undefined> {
  // 1. Try to read from cookies (for Web Client)
  try {
    const cookieStore = await cookies();
    ...
    console.log('[Auth] Cookies not readable or empty');
  }

  // 2. Try to read from Authorization Header (for Mobile Client)
  try {
    const headerList = await headers();
    const authHeader = headerList.get('authorization');
    if (authHeader && authHeader.startsWith('Bearer ')) {
      const token = authHeader.substring(7);
      console.log('[Auth] Found Bearer token in headers');
      return token;
    }
  } catch (e) {
    console.log('[Auth] Headers not readable in this context');
  }
